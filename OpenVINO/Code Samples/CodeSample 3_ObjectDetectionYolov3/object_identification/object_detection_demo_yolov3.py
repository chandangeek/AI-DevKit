...
"""
 ...
 The purpose of the demo is to detect objects for which the model is trained with the confidence level
"""
...
def main():
	...
	# appending the labels to a list named labels_map
    if args.labels:
	...
	#Setting async mode as True
    is_async_mode = True
	#getting the input video into the variable cap using VideoCapture of cv2 
    ...
    while cap.isOpened():
		...
		# Prepare the output frame with inference time, render time, async mode message
		...
		# Display the output in a popup window, together with the detected box and label code
